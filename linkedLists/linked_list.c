...
/* Function RemoveOccurenceByData that remove an element to a linked list by
   a given value.
   This function goes through the whole linked list and compare each data to
   the value until value == data, the memory allocated is then freed for the
   occurence of this data
*/
void RemoveOccurenceByData(struct ListElement list, int value)
{
    ...
    struct Element *elem; //current element
    struct Element *elemN_1 = list.head; // element N-1
    int count = 0; // count the occurence of a data
        ...
        for (elem = list.head; elem != NULL; elem = elem->next)
        {
            if (elem->data == value)
            {
                ...
                if (count > 1)
                {
                    ...
                    {
                        /*
                           if the element to remove is at the begin of the list,
                           the head of the list changes and is linked to the
                           element N+1
                        */
                        list.head = elem->next;
                        elemN_1 = list.head;
                    ...
                    {
                        /*
                           if the element to remove is at the end of the list,
                           the last element is removed and the second to last
                           element becomes the last element and so
                           elemN_1->next = NULL
                        */
                        elemN_1->next = NULL;
                    }
                    ...
                    {
                        /*
                           if the element is somewhere else in the list
                           the element N-1 is linked to the element N+1
                        */
                        elemN_1->next = elem->next;
                    }
                    free(elem);
